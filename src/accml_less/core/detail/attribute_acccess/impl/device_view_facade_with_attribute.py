...
class DeviceViewRFacadeWithAttributes(DeviceViewRFacadeBase):
    def __init__(
        self, *, proxid_object: DeviceViewRFacadeBase, name: Union[str, None] = None
    ):
        self._proxied_obj = proxid_object
        self._name = name
        self._delegates = Delegates(parent=self)
    ...
    def get_view_implementation(
        self, view_implementation_name: str
    ) -> Union[ViewR, ViewRW]:
    ...
    def __dir__(self):
        return list(self._proxied_obj.get_view_implementation_names()) + list(
            super().__dir__()
        )

    def __getattr__(self, item):
        assert (
            item in self.get_view_implementation_names()
        ), f"Request for view implementation {item}, but I only know view implementations {self.get_view_implementation_names()}"
    ...
    def __repr__(self):
        return (
            ...
            f" proxied_object={self._proxied_obj}"
            ")"
        )
...
class DeviceViewRWFacadeWithAttributes(
    DeviceViewRFacadeWithAttributes, DeviceViewRWFacadeBase
):
...
class Delegates:
    def __init__(
        ...
        *,
        parent: Union[
            DeviceViewRWFacadeWithAttributes, DeviceViewRFacadeWithAttributes
        ],
    ):
        self.parent = parent

    def __dir__(self):
        return self.parent.get_view_implementation_names()

    def __getattr__(self, item):
        return self.parent.get_view_implementation(item)

    def __str__(self):
        return f"{self.__class__.__name__}(" f" parent={self.parent.get_name()}" ")"

    def __repr__(self):
        delegates = {
            name: self.parent.get_view_implementation(name)
            for name in self.parent.get_view_implementation_names()
        }
        return f"{self.__class__.__name__}(" f"parent={self.parent}" ")"
