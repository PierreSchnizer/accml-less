...
class ViewWithAttributesProxy(ViewWithAttributesProxyInterface):
    ...
    def get_signal_proxy(self, id_: str) -> Union[SignalProxyRInterface, SignalProxyRWInterface]:
        """
        Todo:
        ...
        if id_ not in attributes:
            raise AttributeUnknownToProxiedObject(f"{id_} unkown, I only know {attributes}")
